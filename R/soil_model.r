...
actET <- function(soil_state,soil_param,meteo)
{
    ...
    {
        potET <- penman(meteo$T,meteo$q,meteo$p,meteo$vpd,meteo$Rn,meteo$G,soil_param$ra)*soil_param$model_step/24
        soilwater <- soil_state$runoff*soil_param$model_step/24
        actet <- min(soilwater,potET)
        new_soil_state <- mutate(soil_state,runoff=runoff-actet)
    } else
    {
        potET <- penmon_day(meteo$T,meteo$q,meteo$p,meteo$vpd,meteo$Rn,meteo$G,soil_param$ra,soil_param$rs)*soil_param$model_step/24
        soilwater <- soil_state$theta*soil_param$soil_depth*10^3 # in mm
        actet <- min(soilwater,potET)
        new_soil_state <- mutate(soil_state,theta=0.001*(soilwater-actet)/soil_param$soil_depth)
    }
    return(new_soil_state)
}
